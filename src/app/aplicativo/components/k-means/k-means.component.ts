...
export class KMeansComponent {
  ...
  exportResultsCSV() {
    ...
    this.downloadFile(csvContent, `resultados_kmeans_k${this.data.result?.k || 3}_${dateStr}.csv`, 'text/csv;charset=utf-8;');
  }
  ...
  exportResultsJSON() {
    ...
    const dateStr = new Date().toISOString().split('T')[0];
    this.downloadFile(jsonContent, `resultados_kmeans_k${this.data.result?.k || 3}_${dateStr}.json`, 'application/json');
  }
  ...
  exportPDFReport() {
    ...
    const k = this.data.result?.k || 3;
    const inercia = (this.data.result?.inercia || 0).toFixed(2);
    const silhouette = this.data.result?.silhouette_score !== undefined
      ? this.data.result.silhouette_score.toFixed(3) : 'N/A';
    const personasCount = this.data.personas?.length || 0;

    // Estilos que Angular inyecta encapsulados – los replicamos en el SVG directamente
    const svgInnerStyle = `
      <style>
        .pca-dot { cursor: pointer; transition: r 0.15s; }
        .pca-dot:hover { r: 9; }
        .axis-label { font-size: 10px; fill: #64748b; font-family: 'Segoe UI', Arial, sans-serif; }
        text { font-size: 11px; fill: #334155; font-family: 'Segoe UI', Arial, sans-serif; }
        circle { stroke-width: 1.5; stroke: rgba(255,255,255,0.6); }
      </style>`;

    // Serializa SVG sin getComputedStyle — los fill ya son atributos inline gracias a Angular [attr.fill]
    const serializeSVG = (svgEl: SVGElement): string => {
      const clone = svgEl.cloneNode(true) as SVGElement;
      const bbox = svgEl.getBoundingClientRect();
      clone.setAttribute('width', String(bbox.width || 560));
      clone.setAttribute('height', String(bbox.height || 280));
      clone.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
      // Insertar estilos básicos dentro del SVG
      clone.insertAdjacentHTML('afterbegin', svgInnerStyle);
      return new XMLSerializer().serializeToString(clone);
    };

    const chartCards = Array.from(document.querySelectorAll('.results-container .chart-card')) as HTMLElement[];
...
<html><head>
<title>Reporte_KMeans_K${k}_${new Date().toISOString().split('T')[0]}</title>
<meta charset="utf-8">
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .svg-wrap svg { width: 100% !important; height: auto !important; max-height: 280px; display: block; }
...
<div class="pdf-header">
  <h1>📊 Reporte K-Means Clustering</h1>
  <p>Generado el ${dateStr} — Sistema Zodíaco</p>
</div>
<div class="pdf-metrics">
  <div class="pdf-metric"><span class="pdf-metric-lbl">Clusters (K)</span><span class="pdf-metric-val">${k}</span></div>
  <div class="pdf-metric"><span class="pdf-metric-lbl">Inercia (WCSS)</span><span class="pdf-metric-val">${inercia}</span></div>
  <div class="pdf-metric"><span class="pdf-metric-lbl">Muestras</span><span class="pdf-metric-val">${personasCount}</span></div>
  <div class="pdf-metric"><span class="pdf-metric-lbl">Silhouette</span><span class="pdf-metric-val">${silhouette}</span></div>
</div>
<div class="section-title">📈 Gráficas y Visualizaciones</div>
${cardsHtml}
<script>window.onload=function(){setTimeout(function(){window.print();},600);};<\/script>
  ...
  getStatsForSelectedCluster() {
    ...
    // Filtrar personas por cluster seleccionado
